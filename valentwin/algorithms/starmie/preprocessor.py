...
# Flatten before preprocessing if list
...
def computeTfIdf(tableDf):
    ...
    for column in tableDf.columns:
        ...
        if len(tableDf[column]) > 0 and type(tableDf[column].tolist()[0]) == list:
            column_values = [item for sublist in tableDf[column].tolist() for item in sublist if item is not None]
            tableDf[column] = pd.Series(column_values)
