...
class TableModel(nn.Module):
    ...
    def __init__(self, device='cuda', lm='roberta-base'):
        super().__init__()
...
class BarlowTwinsSimCLR(nn.Module):
    ...
    """
    def __init__(self, hp, device='cuda', lm='roberta-base'):
        super().__init__()
        self.hp = hp
        self.bert = AutoModel.from_pretrained(lm)
        self.device = device
    ...
    def _extract_columns(self, x, z, cls_indices=None):
        ...
        indices = [idx for idx in indices if idx < len(column_vectors)]  # temporary measure
